...
void saveCredentials(String newServerURL, String newWifiSSID, String newWifiPass) {
...
bool isActivated() {
    return apiKey.length() > 0 && deviceID.length() > 0;
}
...
void initDeviceUID() {
    ...
    char uidBuf[13];
    sprintf(uidBuf, "%02X%02X%02X%02X%02X%02X", 
    ...
    <div class="container">
        ...
        <div class="info">
            <div class="info-item">
                ...
                <div class="value">)" + deviceUID + R"(</div>
            </div>
            <div class="info-item">
                <div class="label">MAC Address</div>
                <div class="value">)" + deviceMAC + R"(</div>
            </div>
            <div class="info-item">
                <div class="label">Firmware</div>
                <div class="value">)" + FIRMWARE_VERSION + R"(</div>
            </div>
            <div class="info-item">
                <div class="label">Model</div>
                <div class="value">)" + DEVICE_MODEL + R"(</div>
            </div>
        </div>
        <form action="/configure" method="POST">
            <input type="text" name="server_url" placeholder="Server URL (e.g., https://datum.example.com)" required>
            <input type="text" name="wifi_ssid" placeholder="WiFi Network Name" required>
            <input type="password" name="wifi_pass" placeholder="WiFi Password">
            <button type="submit">Configure Device</button>
        </form>
        <p class="note">
            📱 Using the Datum mobile app? The app will configure this device automatically.
        </p>
    </div>
</body>
</html>
...
void handleDeviceInfo() {
    ...
    doc["status"] = isActivated() ? "configured" : (hasCredentials() ? "pending" : "unconfigured");
...
void handleConfigure() {
    ...
    if (newServerURL.length() == 0 || newWifiSSID.length() == 0) {
        setupServer.send(400, "application/json", "{\"error\":\"server_url and wifi_ssid are required\"}");
        return;
    }
...
bool activateWithServer() {
    ...
    if (httpCode == 200) {
        ...
        Serial.println("No provisioning request found. Register device via mobile app first.");
    } else if (httpCode == 409) {
        Serial.println("Device already registered. Contact owner for reset.");
    } else if (httpCode == 410) {
        Serial.println("Provisioning request expired. Create new request via mobile app.");
    } else {
