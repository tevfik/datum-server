...
bool tft_output(int16_t x, int16_t y, uint16_t w, uint16_t h,
                uint16_t *bitmap) {
  // Stop if outside screen (should handle clipping, but library does most)
  if (y >= tft.height())
    return 0;
...
void drawCameraView() {
  ...
  if (httpCode == 200) {
    ...
    // Buffer for JPEG (Max 25KB for QVGA/CIF usually fits in efficient heaps,
    // but on 8266 restricted) ESP8266 has ~40KB free heap usually. 20KB buffer
    // is risky but possible. Let's try 15KB limit check or dynamic.
    if (len > 0 && len < 25000) {
      ...
      if (valBuffer) {
        ...
        tft.setSwapBytes(true); // Required for TJpg_Decoder
        // Align image: QVGA (320x240) -> Center crop X?
        // TJpgDec draws at x,y.
        // If we draw at -40, it might crash or clip. Library clips.
        // Let's draw at -40, 0 to center 320 on 240.
...
void setup() {
  ...
  void updateDisplay() {
    ...
    if (currentPropIndex >= properties.size()) {
      if (hasCameraProp) {
        ...
        if (currentPropIndex >
            properties.size() +
                2) { // Show cam for 2 cycles (approx 3s each? poll_interval?)
          currentPropIndex = 0;
        }
        return;
      } else {
  ...
  void parseThingDescription(JsonObject td) {
    ...
    Serial.printf("Parsed %d items (Cam: %d)\n", properties.size(),
                  hasCameraProp);
  }
...
#include <ESP8266httpUpdate.h> // Secure Cloud OTA
...
#define DEVICE_TYPE_NAME "display"
  ...
  struct Config {
    ...
    char ssid[32];
    char password[64];
    char user_token[1024]; // Increased size for tokens
    char device_id[37];
    char api_key[64];
    char server_url[128];
    char target_device_id[37]; // Device to watch
  ...
  // HTML for Onboarding Portal (Preserved from DatumDash)
...
window.onload = function() {
  ...
  void mqttCallback(char *topic, byte *payload, unsigned int length) {
    ...
    if (action == "update_firmware") {
      ...
      if (newTarget.length() > 0) {
        ...
        delay(1000);
        properties.clear(); // Force refresh
        lastPollTime = 0;
      }
    }
  }
  ...
  boolean connectMQTT() {
    ...
    mqttClient.setBufferSize(2048); // Larger buffer for WoT/Dash
    ...
    if (mqttClient.connect(config.device_id, config.device_id,
                           config.api_key)) {
  ...
  // -- Manual Registration (Refined Stable Version) --
  bool registerDeviceManual(String serverUrl, String userToken) {
    ...
    String url = serverUrl + "/dev";
    if (serverUrl.endsWith("/"))
      url = serverUrl + "dev";

    Serial.print("[Manual] Registering at ");
    Serial.println(url);
    ...
    Serial.printf("[Reg] Code: %d, Body: %s\n", code, resp.c_str());
  ...
  void drawCard(String title, String value, String unit) {
    ...
    if (w > 220)
      tft.setTextSize(3);
    tft.getTextBounds(value, 0, 0, &x1, &y1, &w, &h); // Recalc
...
<a href="/reset" class="btn btn-red" onclick="return confirm('Reset?')">⚠️ Factory Reset</a>
  ...
  void startSetupMode() {
    // Stability Fixes for Mobile App Connectivity
    WiFi.disconnect();
    delay(100);
    WiFi.mode(WIFI_AP); // Strictly AP Mode for setup (avoids channel hopping)

    // Force IP to standard 192.168.4.1
    ...
    server.enableCORS(true); // Allow Mobile App (WebView) access
    ...
    // Mobile App Device Info Endpoint
    server.on("/info", []() {
      String mac = WiFi.macAddress();
      String uid = "DatumDash-" + String(ESP.getChipId(), HEX);
      ...
      json += "\"firmware_version\":\"" + String(FIRMWARE_VER) + "\",";
    ...
    server.on("/save", HTTP_POST, []() {
      ...
      if (token.length() == 0 && email.length() > 0) {
        ...
        http.addHeader("Content-Type", "application/json");
        int c = http.POST("{\"email\":\"" + email + "\",\"password\":\"" +
                          upass + "\"}");
        if (c == 200) {
          DynamicJsonDocument doc(512);
      ...
      // Clear old Credentials to force re-register
