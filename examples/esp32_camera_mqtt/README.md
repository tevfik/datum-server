# ESP32 Camera Streaming with Datum Server

Complete guide for setting up video streaming from ESP32 camera boards to Datum server using WebSocket and MJPEG protocols.
...
This example demonstrates real-time camera streaming from ESP32 behind NAT/proxy to web clients using Datum server as a relay.
...
> **Note**: ESP32 is recommended over ESP32-S2 for camera applications. The ESP32-S2 has limited camera interface support and may have compatibility issues with some sensors.
...
Most ESP32 Camera boards (especially Freenove S3 Cam and generic clones) use **8MB OPI PSRAM**.
...
### 1. ESP32-CAM (Seeed XIAO, Waveshare)
```
...
```
┌──────────────────┐         ┌──────────────────┐         ┌──────────────────┐
│   ESP32 CAM   │         │  Datum Server    │         │   Web Browser    │
...
```
http://your-server:8000/dev/esp32-cam-01/stream/mjpeg?token=YOUR_JWT
```

**Option C: VLC Player**
```bash
vlc http://your-server:8000/dev/esp32-cam-01/stream/mjpeg?token=YOUR_JWT
```
...
curl http://localhost:8000/dev/esp32-cam-01/stream/info

# Test through Traefik
curl https://datum.example.com/dev/esp32-cam-01/stream/info
```
