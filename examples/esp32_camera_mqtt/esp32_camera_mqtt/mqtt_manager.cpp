...
void mqttCallback(char *topic, byte *payload, unsigned int length) {
  ...
  if (pid.length() > 0 && action.length() > 0) {
    ...
    if (action == "update_settings") {
        applyRemoteConfig(paramsBlock);
        return;
    }
    // ...
  } else if (String(topic).endsWith("conf/set")) {
      // Remote Config Message
      String desired = "";
      int dStart = pl.indexOf("\"desired\":");
      if (dStart > 0) {
          int dvalStart = pl.indexOf('{', dStart);
          if (dvalStart > 0) {
              int dend = dvalStart;
              int dcount = 1;
              while (dcount > 0 && dend < pl.length() - 1) {
                  dend++;
                  if (pl.charAt(dend) == '{') dcount++;
                  else if (pl.charAt(dend) == '}') dcount--;
              }
              desired = pl.substring(dvalStart, dend + 1);
          }
      }
      if (desired.length() > 0) {
          Serial.println("[MQTT] Received Config Update");
          applyRemoteConfig(desired);
  ...
  // Legacy Command Logic (Keep for backward compatibility if needed)
  if (pid.length() > 0 && action.length() > 0) {
...
bool reconnectMQTT() {
  ...
  if (mqttClient.connect(clientId.c_str(), deviceID.c_str(), apiKey.c_str())) {
    ...
    // Subscribe to conf/set for Remote Config
    String topicConf = "dev/" + deviceID + "/conf/set";
    mqttClient.subscribe(topicConf.c_str());
    
    Serial.println("[MQTT] Subscribed to " + topic + " and " + topicConf);
...
void applyRemoteConfig(String paramsBlock) {
    ...
    if (paramsBlock.indexOf("\"stream_enabled\":") != -1) {
        ...
        if (enabled) {
            ...
            prefs.end();
            streaming = true;
            Serial.println("Streaming STARTED via config");
        } else {
            prefs.begin("datum", false);
            prefs.putBool("pref_stream_en", false);
            prefs.end();
            streaming = false;
            Serial.println("Streaming STOPPED via config");
        }
    }

    // Stream Resolution
    String vres = extractJsonVal(paramsBlock, "vres");
    if (vres.length() == 0) vres = extractJsonVal(paramsBlock, "stream_resolution");
    if (vres.length() > 0) {
        // Only save preference. Requires reboot to apply resolution change in some camera drivers, 
        // or re-init in main loop. 
        prefs.begin("datum", false);
        prefs.putString("pref_vres", vres);
        prefs.end();
    }
    ...
    if (lcol.length() == 0) lcol = extractJsonVal(paramsBlock, "led_color");
    if (lcol.length() > 0) {
        if (lcol.startsWith("#")) {
            ...
            // Apply immediately if defined
            #ifdef LED_GPIO_NUM
            neopixelWrite(LED_GPIO_NUM, savedR, savedG, savedB);
            #endif
        }
    }
    ...
    if (lbri == -1) lbri = extractJsonInt(paramsBlock, "led_brightness");
    if (lbri != -1) {
        savedBrightness = lbri;
        prefs.begin("datum", false);
        prefs.putInt("pref_lbri", savedBrightness);
        prefs.end();
        // Adjust current color if needed logic exists
    }
}

// Fetch Config via HTTP (Fallback / Boot Sync)
void fetchRemoteConfig() {
    if (WiFi.status() != WL_CONNECTED || deviceID.length() == 0 || apiKey.length() == 0) return;

    HTTPClient http;
    // GET /dev/{id} returns the device object including desired_state
    String url = serverURL + "/dev/" + deviceID;
    
    // If using HTTPS, setInsecure might be needed depending on CA cert availability
    // http.setInsecure(); 
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        // Extract desired_state object
        // The structure is {"devices": [...]} or just object? 
        // Backend getDeviceHandler returns {"devices": [DeviceResponse]} wrapper? 
        // Let's check getDeviceHandler implementation. 
        // It returns c.JSON(http.StatusOK, gin.H{"devices": response}) which is an ARRAY for list, 
        // BUT getDeviceHandler returns DeviceResponse OBJECT directly?
        // Checking getDeviceHandler in handlers_device.go:
        // c.JSON(http.StatusOK, resp) -> This sends the object directly!
        
        // Find desired_state
        String desired = "";
        int dStart = payload.indexOf("\"desired_state\":");
        if (dStart > 0) {
           int dvalStart = payload.indexOf('{', dStart);
           if (dvalStart > 0) {
               int dend = dvalStart;
               int dcount = 1;
               while (dcount > 0 && dend < payload.length() - 1) {
                   dend++;
                   if (payload.charAt(dend) == '{') dcount++;
                   else if (payload.charAt(dend) == '}') dcount--;
               }
               desired = payload.substring(dvalStart, dend + 1);
           }
        }
        
        if (desired.length() > 0 && desired != "null") {
             Serial.println("[HTTP] Syncing Remote Config...");
             applyRemoteConfig(desired);
        }
    } else {
        Serial.printf("[HTTP] Config Sync Failed: %d\n", httpCode);
    }
    http.end();
}
