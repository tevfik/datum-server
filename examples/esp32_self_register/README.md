...
## Mobile App Integration (SoftAP Concept)

You asked about the interaction with the Mobile Application via SoftAP. While this example uses hardcoded credentials, you can combine strict Self-Registration with the SoftAP mechanism from the Provisioning example.

In that "Hybrid" flow, the Mobile App connects to the device's WiFi and "pushes" the User Token and WiFi Credentials to it.

```
┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│    Device   │     │  Mobile App │     │ Datum Server│
│   (SoftAP)  │     │             │     │             │
└──────┬──────┘     └──────┬──────┘     └──────┬──────┘
       │                   │                   │
       │ 1. Start SoftAP   │                   │
