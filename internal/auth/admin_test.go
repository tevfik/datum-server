...
func TestCheckUserStatusActive(t *testing.T) {
	store, cleanup := createTestStorageForAuth(t)
	defer cleanup()
	...
	err := CheckUserStatus(store, "user123")
	assert.NoError(t, err)
}

func TestCheckUserStatusSuspended(t *testing.T) {
	store, cleanup := createTestStorageForAuth(t)
	defer cleanup()
...
func TestCheckUserStatusNotFound(t *testing.T) {
	store, cleanup := createTestStorageForAuth(t)
	defer cleanup()

	err := CheckUserStatus(store, "nonexistent")
	assert.Error(t, err)
}
